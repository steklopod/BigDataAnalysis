package ru.steklopod.cassandra

import com.datastax.spark.connector._
import ru.steklopod.cassandra.model.{KV, UserData}
...
class CassandraSparkConnector {
  ...
  def joinWithCassandraKVTAbleWithRepartition(sparkSession: SparkSession,
                                              toJoin: RDD[KV]) = {
    repartitionByReplica(sparkSession, toJoin)
      .joinWithCassandraTable("test", "kv")
      .on(SomeColumns("key"))
  }
