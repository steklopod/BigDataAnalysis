...
object FindAssociationRules extends Logging {
  ...
  def findCountsOfOccurringSequences(
      transactions: RDD[String]): RDD[(List[String], Int)] = {
    val patterns = transactions
      .flatMap(line => {
  ...
  def performMBAAnalysis(transactions: RDD[String]) = {
    ...
    val combined: RDD[(List[String], Int)] = findCountsOfOccurringSequences(
      transactions)
