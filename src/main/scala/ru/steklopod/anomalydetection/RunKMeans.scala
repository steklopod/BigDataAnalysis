package ru.steklopod.anomalydetection
...
class RunKMeans(private val spark: SparkSession) {
  ...
  def transformColumnToVectorOfFeatures(
      inputCol: String): (Pipeline, String) = {
  ...
  def buildMlPipeline(data: DataFrame, k: Int): PipelineModel = {
    val (protoTypeEncoder, protoTypeVecCol) = transformColumnToVectorOfFeatures(
      "protocol_type")
    val (serviceEncoder, serviceVecCol) = transformColumnToVectorOfFeatures(
      "service")
  ...
  def buildAnomalyDetector(data: DataFrame): Unit = {
    ...
    val farthestDistanceBetweenTwoNormalClusters: Double =
      getDistanceOfTwoClustersThatAreMostAwayFromEachOtherFromNTopClusters(
        clustered,
        100,
        centroids)

    val originalCols = data.columns
    val anomalies: DataFrame = findAnomalies(
      centroids,
      clustered,
      farthestDistanceBetweenTwoNormalClusters,
      originalCols)
